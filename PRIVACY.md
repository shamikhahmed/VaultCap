# VaultOS — Privacy Policy

**Last updated:** June 10, 2026

VaultOS is an offline-first Progressive Web App. **Your vault data stays on your device** unless you explicitly export it.
...
VaultOS offers three parsing paths for Smart Add:

1. **Smart Parser (default, offline)** — Rule-based parsing on-device. No network. No data leaves your browser.
2. **Bundled LLM (shipped)** — VaultOS includes `js/config/llm-bundled.js` with a proxy provider. Requests go to the Cloudflare Worker at `https://vaultos-llm-proxy.shamikhahmed.workers.dev` (`POST /parse`). Only the text you submit for parsing is transmitted — never your full vault, PIN, or encryption keys.
...
- VaultOS is a general-purpose adult life OS. Parents should supervise device sharing and exports on shared family devices.
