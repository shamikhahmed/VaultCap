  ...
  showSwitcher() {
    ...
    Modal.open('Vault Profiles',
      ...
      VaultProfiles.pickerProfiles().map(p =>
        '<div data-act="VaultProfiles.switch(\'' + p.id + '\')" style="display:flex;align-items:center;gap:12px;padding:14px;background:' + (p.id===active?'rgba(123,95,255,.12)':'var(--glass)') + ';border:1px solid ' + (p.id===active?'rgba(123,95,255,.4)':'var(--border)') + ';border-radius:14px;cursor:pointer;touch-action:manipulation">' +
