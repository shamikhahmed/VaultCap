...
const Cards={
  ...
  openAdd(prefill={}){
    ...
    const scanBtn = prefill.ownerId ? '' : `<div style="margin-bottom:12px"><button class="btn btn-g btn-full" onclick="Cards.scanCard()" style="gap:8px">📷 Scan Card with Camera</button></div>`;
  ...
  form(c={}){
    ...
    return '<datalist id="cfNameDL">'+cardNames+'</datalist>'
      ...
      +'<div class="fg"><label class="fl">Joint Card?</label><div style="display:flex;align-items:center;gap:12px;margin-top:6px"><input type="checkbox" id="cf-joint" onchange="var s=document.getElementById(\'cf-joint-section\');if(s)s.style.display=this.checked?\'block\':\'none\'" style="width:20px;height:20px;cursor:pointer" '+(c.jointAccount?'checked':'')+' ><label for="cf-joint" style="font-size:14px;color:var(--text2)">This is a joint card</label></div><div id="cf-joint-section" style="display:'+(c.jointAccount?'block':'none')+';margin-top:10px"><select id="cf-joint-person" style="width:100%;background:var(--input);border:1px solid var(--border);border-radius:10px;padding:12px;color:var(--text);font-size:14px"><option value="">Select person...</option>'+(()=>{try{const fam=JSON.parse(localStorage.getItem('vo_family')||'{}');const members=(fam.members||[]).map(m=>`<option value="family:${m.name}"${c.jointWith==='family:'+m.name?' selected':''}>${m.name} (Family)</option>`).join('');const contacts=(typeof S!=='undefined'?S.friends||[]:[]).map(f=>`<option value="contact:${f.name}"${c.jointWith==='contact:'+f.name?' selected':''}>${f.name} (Contact)</option>`).join('');return members+contacts;}catch(e){return '';}})()+'</select></div></div>'
