...
const Banks={
  ...
  row(b){
    ...
    return `<div class="entry" data-id="${b.id}"><div class="entry-main"><div class="entry-ic" style="padding:0;overflow:hidden;border-radius:10px;flex-shrink:0">${logoHtml}</div><div class="entry-body"><div class="entry-name">${b.bankName}${b.ownership==='business'?' <span class="badge b-warn">Business</span>':''}${b.jointAccount&&b.jointWith?' <span class="badge b-muted">Joint: '+(b.jointWith.split(':')[1]||b.jointWith)+'</span>':''}${_sharedBadge}</div><div class="entry-sub">${b.accountType||''} · ${b.currency||''} ${b.last4?'· ****'+b.last4:''}</div><div class="entry-meta"><span class="badge b-muted">${b.bankType||'bank'}</span>${b.twoFA?'<span class="badge b-ok">2FA</span>':''} ${b.tags?.slice(0,2).map(t=>`<span class="badge b-muted">${t}</span>`).join('')||''}</div></div><div class="entry-acts">${U.icb('star',{onclick:`Banks.fav('${b.id}')`,class:'fav'+(b.favorite?' on':'')})}${U.icb('eye',{onclick:`Banks.detail('${b.id}')`,ariaLabel:'View details'})}${U.icb('pencil',{onclick:`Banks.edit('${b.id}')`,ariaLabel:'Edit'})}${U.icb('archive',{onclick:`Banks.archive('${b.id}')`,title:b.archived?'Unarchive':'Archive'})}${U.icb('trash',{onclick:`Banks.del('${b.id}')`,ariaLabel:'Delete',class:'del'})}</div></div></div>`;
  },
