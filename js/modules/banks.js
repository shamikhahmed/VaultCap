...
const Banks={
  ...
  render(){
    ...
    const archiveToggle=archivedCount?`<div style="text-align:center;margin-bottom:10px"><button type="button" class="btn btn-g btn-sm" data-act="Banks._showArchived=!Banks._showArchived;Banks.render()">${Banks._showArchived?'Hide':'Show'} ${archivedCount} archived</button></div>`:'';
  ...
  form(b={}){
    ...
    <div id="bf-adv" style="display:${isEdit?'flex':'none'};flex-direction:column;gap:10px">
      ...
      <div class="fg"><label class="fl">Joint Account?</label><div style="display:flex;align-items:center;gap:12px;margin-top:6px"><input type="checkbox" id="bf-joint" data-act-change="Banks._toggleJoint(this.checked)" style="width:20px;height:20px;cursor:pointer" ${b.jointAccount?'checked':''}><label for="bf-joint" style="font-size:14px;color:var(--text2)">This is a joint account</label></div><div id="b-joint-section" style="display:${b.jointAccount?'block':'none'};margin-top:10px"><div style="font-size:12px;color:var(--text3);margin-bottom:8px">Joint with (select from Family or Contacts):</div><select id="b-joint-person" style="width:100%;background:var(--input);border:1px solid var(--border);border-radius:10px;padding:12px;color:var(--text);font-size:14px"><option value="">Select person...</option>${typeof familyJointOptionsHtml==='function'?familyJointOptionsHtml(b.jointWith||''):''}</select><div style="font-size:11px;color:var(--text3);margin-top:6px;padding:8px;background:rgba(0,213,255,.06);border-radius:8px;border:1px solid rgba(0,213,255,.2)">Joint accounts count 100% toward your net worth — avoiding double-counting.</div></div></div>
