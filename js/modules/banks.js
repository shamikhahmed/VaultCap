const Banks={
  ...
  row(b){
    ...
    const logoHtml=dom
      ? `<img src="https://www.google.com/s2/favicons?domain=${dom}&sz=64" width="36" height="36" style="border-radius:8px;object-fit:cover;display:block" onerror="this.outerHTML='<div style=\\'width:36px;height:36px;border-radius:8px;background:${color};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:#fff\\'>${initials}</div>'">`
      : `<div style="width:36px;height:36px;border-radius:8px;background:${color};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:#fff">${initials}</div>`;
  ...
  emptyState(){return `<div class="empty-ios"><div class="ei-ic">🏦</div><div class="ei-title">No Banks Yet</div><div class="ei-sub">Add your bank accounts to track balances, IBANs, and login details</div><button class="btn btn-p" onclick="Banks.openAdd()">+ Add Bank</button><button class="btn btn-g" onclick="Banks._showExample()" style="width:100%;max-width:260px;margin-top:10px">See example</button></div>`;},
      ...
      <details style="margin-bottom:10px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(80px,1fr));gap:7px;margin-top:8px" id="bf-tiles-grid">
          ${tiles.map(b=>{
            ...
            const logoEl=dom
              ? `<img src="https://www.google.com/s2/favicons?domain=${dom}&sz=64" width="28" height="28" style="border-radius:6px;object-fit:cover;display:block" onerror="this.outerHTML='<div style=\\'width:28px;height:28px;border-radius:6px;background:${color};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:#fff\\'>${initials}</div>'">`
              : `<div style="width:28px;height:28px;border-radius:6px;background:${color};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:#fff">${initials}</div>`;
