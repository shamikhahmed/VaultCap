const Cash = {
  render() {
    ...
    el.innerHTML = data.map(c => `<div class="entry"><div class="entry-main"><div class="entry-ic">${locIc[c.location] || '💵'}</div><div class="entry-body"><div class="entry-name">${escHtml(c.location || 'Cash')}</div><div class="entry-sub sens">${(c.amount || 0).toLocaleString()} ${escHtml(c.currency || '')}${c.notes ? ' · ' + escHtml(c.notes) : ''}</div><div class="entry-meta"><span class="badge ${locColor[c.location]||'b-muted'} sens">${(c.amount || 0).toLocaleString()} ${escHtml(c.currency || '')}</span>${(c.tags||[]).slice(0,2).map(t=>`<span class="badge b-muted">${escHtml(t)}</span>`).join('')}</div></div><div class="entry-acts"><button type="button" class="icb" title="Transfer" onclick="Cash.transfer('${c.id}')">→</button><button type="button" class="icb" aria-label="Edit" onclick="Cash.edit('${c.id}')">✏️</button><button type="button" class="icb del" aria-label="Delete" onclick="Cash.del('${c.id}')">🗑️</button></div></div></div>`).join('');
  },
