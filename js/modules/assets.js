...
const ASSET_TYPES_MAP = {
  property:        { label:'Property',        ic:'building', color:VCBrand.h_e8e8e8 },
  vehicle:         { label:'Vehicle',         ic:'car', color:VCBrand.h_d0d0d0 },
  electronics:     { label:'Electronics',     ic:'laptop', color:VCBrand.h_b8b8b8 },
  gadget:          { label:'Gadget',          ic:'laptop', color:VCBrand.h_b8b8b8 },
  precious_metals: { label:'Precious Metals', ic:'gem', color:VCBrand.h_a0a0a0 },
  precious:        { label:'Precious Metals', ic:'gem', color:VCBrand.h_a0a0a0 },
  watch:           { label:'Watch',           ic:'watch', color:VCBrand.h_888888 },
  jewelry:         { label:'Jewellery',       ic:'ring', color:VCBrand.h_707070 },
  subscription:    { label:'Subscription',   ic:'repeat', color:VCBrand.h_636366 },
  insurance:       { label:'Insurance',      ic:'shield', color:VCBrand.h_585858 },
  business:        { label:'Business',       ic:'building-2', color:VCBrand.h_484848 },
  loan:            { label:'Loan Asset',     ic:'banknote', color:VCBrand.h_383838 },
  other:           { label:'Other',          ic:'package', color:VCBrand.h_9e9e9e }
};
...
const Assets = {
  ...
  render() {
    ...
    if (ci) ci.innerHTML = chips.map(([v,l,ic]) => `<div class="chip${v===f?' on':''}" data-act="S.aF='${v}';Assets.render()"><span class="chip-ic">${VC.icon(ic, 12)}</span>${l}</div>`).join('');
    ...
    if (smEl) {
      ...
      if (withPnl.length) {
        ...
        smEl.innerHTML = `<div class="widget vc-ix-9"><div class="wh"><span class="vc-icon-wrap">${VC.icon('trending-up', 16)}</span>Portfolio P&amp;L</div><div class="vc-ix-226"><div class="vc-ix-227"><div class="vc-ix-24">Cost basis</div><div class="sens vc-ix-137">${U.fmt(Math.round(fromB(totalCost, cur)))} <span style="font-size:11px;font-weight:400;color:var(--text3)">${cur}</span></div></div><div class="vc-ix-228"><div class="vc-ix-24">Current value</div><div style="font-size:18px;font-weight:900;color:var(--accent)" class="sens">${U.fmt(Math.round(fromB(totalVal, cur)))}</div></div><div class="vc-ix-229"><div class="vc-ix-24">Gain / Loss</div><div style="font-size:18px;font-weight:900;color:${pnlColor}" class="sens">${pnl >= 0 ? '+' : '−'}${U.fmt(Math.round(Math.abs(fromB(pnl, cur))))} <span class="vc-ix-231">(${pnl >= 0 ? '+' : ''}${pnlPct}%)</span></div></div></div><div style="font-size:11px;color:var(--text3);padding:6px 14px 0;border-top:1px solid var(--border)">${withPnl.length} asset${withPnl.length !== 1 ? 's' : ''} with purchase price</div></div>`;
      } else { smEl.innerHTML = ''; }
    }
    ...
    if (!data.length) {
      ...
      el.innerHTML = `<div class="empty-ios"><div class="ei-ic">${VC.assetIcon(f==='all'?'other':f, 32)}</div><div class="ei-title">No ${typeInfo.label} Yet</div><div class="ei-sub">Track property, vehicles, electronics, metals and more</div><button type="button" class="btn btn-p" data-act="Assets.openAdd('${f==='all'?'':f}')">+ Add Asset</button></div>`;
      return;
    }
    const cur = S.user.currency || 'PKR';
    el.innerHTML = data.map(a => {
      ...
      return `<div class="entry"><div class="entry-main"><div class="entry-ic">${VC.assetIcon(a.assetType, 18)}</div><div class="entry-body"><div class="entry-name">${escHtml(a.name||'Asset')}</div><div class="entry-sub">${sub||escHtml((a.notes||'').slice(0,60))}</div><div class="entry-meta">${valDisplay?`<span class="badge b-acc">${displayCur} ${U.fmt(valDisplay)}</span>`:''} ${_pnlHtml} <span class="badge b-muted">${a.ownership==='business'?'Business':'Personal'}</span>${a.assetType==='vehicle'&&a.staffAssigned?` <span class="badge b-muted">${escHtml(a.staffAssigned)}</span>`:''}</div></div><div class="entry-acts">${U.icb('star',{onclick:`Assets.fav('${a.id}')`,class:'fav'+(a.favorite?' on':'')})}${U.actsViewEditDel('Assets', a.id)}</div></div></div>`;
    }).join('');
  },
  ...
  openAdd(typeOrPrefill) {
    ...
    Modal.open(title, this.form(preType ? {assetType:preType} : {}), `<button type="button" class="btn btn-g" data-act="Modal.close()">Cancel</button><button type="button" class="btn btn-p" data-act="Assets.save()">Save</button>`);
    setTimeout(() => {
  ...
  form(a = {}) {
    ...
    <div class="vc-ix-27" id="af-sub"></div>
    <div class="fg vc-ix-52"><label class="fl">Notes</label><textarea class="inp" id="af-notes" rows="2">${escHtml(a.notes||'')}</textarea></div>
    <div class="fg"><label class="fl">Tags</label>${U.tags(a.tags||[])}</div>
    <label class="vc-ix-128"><input type="checkbox" id="af-fav" ${a.favorite?'checked':''}> Favourite</label>`;
  },
  ...
  detail(id) {
    ...
    const docsHtml = relatedDocs.length
      ? `<div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)"><div class="vc-ix-37">Related Documents (${relatedDocs.length})</div>${relatedDocs.map(d=>`<div class="vc-ix-85"><span class="chip-ic">${VC.icon('file',16)}</span><div><div class="vc-ix-12">${d.name||d.docName||'Document'}</div><div class="vc-ix-3">${d.docType||''} ${d.expiryDate?'· Exp '+d.expiryDate:''}</div></div></div>`).join('')}</div>`
      : '';
    const remHtml = relatedReminders.length
      ? `<div class="vc-ix-240"><div class="vc-ix-37">Linked Reminders (${relatedReminders.length})</div>${relatedReminders.map(r=>`<div class="vc-ix-85"><span class="chip-ic">${VC.icon('bell',16)}</span><div><div class="vc-ix-12">${r.name||'Reminder'}</div><div class="vc-ix-3">${r.renewalDate||''}</div></div></div>`).join('')}</div>`
      : '';
    ...
    Modal.open(`${typeInfo.label} — ${a.name||'Asset'}`,
      ...
      `<button type="button" class="btn btn-g" data-act="Modal.close()">Close</button><button type="button" class="btn btn-p" data-act="Assets.edit('${id}');Modal.close()">Edit</button>`
    );
  },

  edit(id) {
    const a = (S.assets||[]).find(x => x.id===id); if (!a) return;
    Modal.open('Edit Asset', this.form(a), `<button type="button" class="btn btn-g" data-act="Modal.close()">Cancel</button><button type="button" class="btn btn-d btn-sm" data-act="Assets.del('${id}',true)">Delete</button><button type="button" class="btn btn-p" data-act="Assets.save('${id}')">Update</button>`);
    setTimeout(() => {
