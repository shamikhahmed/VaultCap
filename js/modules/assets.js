...
const Assets = {
  ...
  detail(id) {
    ...
    const remHtml = relatedReminders.length
      ? `<div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border)"><div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text3);margin-bottom:8px">Linked Reminders (${relatedReminders.length})</div>${relatedReminders.map(r=>`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)"><span class="chip-ic">${VC.icon('bell',16)}</span><div><div style="font-size:13px;font-weight:600;color:var(--text)">${r.name||'Reminder'}</div><div style="font-size:11px;color:var(--text3)">${r.renewalDate||''}</div></div></div>`).join('')}</div>`
      : '';
