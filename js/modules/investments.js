const Inv={
  render(){
    ...
    if(!data.length){el.innerHTML=`<div class="empty-ios"><div class="ei-ic">📈</div><div class="ei-title">No investments yet</div><div class="ei-sub">Track stocks, mutual funds, crypto, NSS bonds — multi-currency portfolio in one place</div><div style="display:flex;gap:10px;justify-content:center;margin-top:16px;flex-wrap:wrap"><button type="button" class="btn btn-p" onclick="Inv.openAdd()">+ Add Investment</button></div></div>`;return;}
    el.innerHTML=data.map(i=>{const r={Low:'b-ok',Medium:'b-warn',High:'b-err'};const ic={Stocks:'📊','Mutual Funds':'📁',ETFs:'📈',Bonds:'📜',Sukuk:'🕌',Crypto:'₿','Fixed Deposit':'🏛️',Pension:'👴',Other:'💼'};const hasBoth=i.amountInvested&&i.currentValue;const invLine=hasBoth?`<div style="font-size:11px;color:var(--text3);margin-top:2px">Inv: <span class="sens">${i.currency} ${Math.round(i.amountInvested).toLocaleString()}</span> · Cur: <span class="sens">${i.currency} ${Math.round(i.currentValue).toLocaleString()}</span></div>`:'';return `<div class="entry" data-id="${i.id}"><div class="entry-main"><div class="entry-ic">${ic[i.type]||'📈'}</div><div class="entry-body"><div class="entry-name">${i.investmentName||i.broker||'Investment'}</div><div class="entry-sub">${i.broker||''} · ${i.type||''} · ${i.currency||''}</div><div class="entry-meta">${i.riskLevel?`<span class="badge ${r[i.riskLevel]||'b-muted'}">${i.riskLevel}</span>`:''} ${hasBoth?`<span style="font-size:11px">${U.pnl(i.amountInvested,i.currentValue)}</span>`:i.currentValue?`<span class="badge b-acc">${U.fmt(i.currentValue)}</span>`:''} ${i.ownership==='business'?'<span class="badge b-warn">🏢</span>':''} ${(i.tags||[]).slice(0,2).map(t=>`<span class="badge b-muted">${t}</span>`).join('')}</div>${invLine}</div><div class="entry-acts"><button type="button" class="icb fav${i.favorite?' on':''}" onclick="Inv.fav('${i.id}')">⭐</button><button type="button" class="icb" aria-label="Edit" onclick="Inv.edit('${i.id}')">✏️</button><button type="button" class="icb del" aria-label="Delete" onclick="Inv.del('${i.id}')">🗑️</button></div></div></div>`;}).join('');
    initSwipeDelete(el);
  },
