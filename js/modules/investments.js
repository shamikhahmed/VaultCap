const Inv={
  ...
  openAdd(prefill={}){
    ...
    Modal.open(title,this.form(),`<button class="btn btn-g" onclick="Modal.close()">Cancel</button><button class="btn btn-p" onclick="Inv.save()">Save</button>`);setTimeout(()=>{const c=S.user.currency||'PKR';['if-inv','if-cur2'].forEach(id=>{const el=document.getElementById(id);if(el)U.numInput(el,c);});},60);
  },
  form(i={}){
    const isEdit=!!i.id;
