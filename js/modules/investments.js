const Inv={
  ...
  edit(id){const i=S.investments.find(x=>x.id===id);if(!i)return;Modal.open('✏️ Edit Investment',this.form(i),`<button type="button" class="btn btn-g" onclick="Modal.close()">Cancel</button><button type="button" class="btn btn-d btn-sm" onclick="Inv.del('${id}',true)">Delete</button><button type="button" class="btn btn-p" onclick="Inv.save('${id}')">Update</button>`);setTimeout(()=>{[['if-type',i.type||'Stocks'],['if-risk',i.riskLevel||'Medium'],['if-cur',i.currency||'GBP'],['if-cc',i.country||'GB'],['if-own',i.ownership||'personal']].forEach(([el,v])=>{const e=document.getElementById(el);if(e)e.value=v;});U.setLF(i);const c=i.currency||'GBP';['if-inv','if-cur2'].forEach(id=>{const el=document.getElementById(id);if(el)U.numInput(el,c);});},60);},
  lookupTicker(ticker){
