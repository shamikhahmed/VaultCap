const Inv={
  render(){
    ...
    if(sm){
      ...
      sm.innerHTML=S.investments.length?`<div class="widget" style="margin-bottom:12px"><div class="fr" style="gap:0"><div style="flex:1;text-align:center"><div style="font-size:10px;color:var(--text3)">Invested</div><div style="font-size:18px;font-weight:700" class="sens">${U.fmtCur(tiPKR,cur)}</div></div><div style="flex:1;text-align:center"><div style="font-size:10px;color:var(--text3)">Current</div><div style="font-size:18px;font-weight:700" class="sens">${U.fmtCur(tcPKR,cur)}</div></div><div style="flex:1;text-align:center"><div style="font-size:10px;color:var(--text3)">P&L</div><div style="font-size:16px">${tiPKR>0?U.pnl(tiPKR,tcPKR):'—'}</div></div></div></div>`:'';
    }
    const el=document.getElementById('invItems');if(!el)return;
    if(!data.length){el.innerHTML=`<div class="empty-ios"><div class="ei-ic">${VC.icon('trending-up',32)}</div><div class="ei-title">No investments yet</div><div class="ei-sub">Track stocks, mutual funds, crypto, NSS bonds — multi-currency portfolio in one place</div><div style="display:flex;gap:10px;justify-content:center;margin-top:16px;flex-wrap:wrap"><button type="button" class="btn btn-p" data-act="Inv.openAdd()">+ Add Investment</button></div></div>`;return;}
    el.innerHTML=data.map(i=>{const r={Low:'b-ok',Medium:'b-warn',High:'b-err'};const hasBoth=i.amountInvested&&i.currentValue;const invLine=hasBoth?`<div style="font-size:11px;color:var(--text3);margin-top:2px">Inv: <span class="sens">${i.currency} ${Math.round(i.amountInvested).toLocaleString()}</span> · Cur: <span class="sens">${i.currency} ${Math.round(i.currentValue).toLocaleString()}</span></div>`:'';return `<div class="entry" data-id="${i.id}"><div class="entry-main"><div class="entry-ic">${VC.investIcon(i.type,18)}</div><div class="entry-body"><div class="entry-name">${i.investmentName||i.broker||'Investment'}</div><div class="entry-sub">${i.broker||''} · ${i.type||''} · ${i.currency||''}</div><div class="entry-meta">${i.riskLevel?`<span class="badge ${r[i.riskLevel]||'b-muted'}">${i.riskLevel}</span>`:''} ${hasBoth?`<span style="font-size:11px">${U.pnl(i.amountInvested,i.currentValue)}</span>`:i.currentValue?`<span class="badge b-acc">${U.fmt(i.currentValue)}</span>`:''} ${i.ownership==='business'?'<span class="badge b-warn">Business</span>':''} ${(i.tags||[]).slice(0,2).map(t=>`<span class="badge b-muted">${t}</span>`).join('')}</div>${invLine}</div><div class="entry-acts">${U.icb('star',{onclick:`Inv.fav('${i.id}')`,class:'fav'+(i.favorite?' on':'')})}${U.actsEditDel('Inv', i.id)}</div></div></div>`;}).join('');
    initSwipeDelete(el);
  },
