const Inv={
  render(){
    ...
    const chips=[['all','All','chart'],['Stocks','Stocks','chart'],['Mutual Funds','Funds','layers'],['Bonds','Bonds','ticket'],['Crypto','Crypto','target'],['Fixed Deposit','FD','bank'],['fav','Favorites','star']];
    const ci=document.getElementById('invChips');if(ci)ci.innerHTML=chips.map(([v,l,ic])=>`<div class="chip${v===f?' on':''}" data-act="S.invF='${v}';Inv.render()"><span class="chip-ic">${VC.icon(ic,12)}</span>${l}</div>`).join('');
    ...
    if(sm){
      const toPKR=(a,c)=>typeof CurrencyEngine!=='undefined'?CurrencyEngine.toBase(a||0,c||'PKR'):(a||0);
      const cur=S.user.currency||'PKR';
      const tiPKR=S.investments.reduce((a,i)=>a+toPKR(i.amountInvested||0,i.currency),0);
      const tcPKR=S.investments.reduce((a,i)=>a+toPKR(i.currentValue||0,i.currency),0);
      sm.innerHTML=S.investments.length?`<div class="widget vc-ix-9"><div class="fr vc-ix-230"><div class="vc-ix-127"><div class="vc-ix-18">Invested</div><div class="sens vc-ix-84">${U.fmtCur(tiPKR,cur)}</div></div><div class="vc-ix-127"><div class="vc-ix-18">Current</div><div class="sens vc-ix-84">${U.fmtCur(tcPKR,cur)}</div></div><div class="vc-ix-127"><div class="vc-ix-18">P&L</div><div style="font-size:16px">${tiPKR>0?U.pnl(tiPKR,tcPKR):'—'}</div></div></div></div>`:'';
    }
    const el=document.getElementById('invItems');if(!el)return;
    if(!data.length){el.innerHTML=`<div class="empty-ios"><div class="ei-ic">${VC.icon('trending-up',32)}</div><div class="ei-title">No investments yet</div><div class="ei-sub">Track stocks, mutual funds, crypto, NSS bonds — multi-currency portfolio in one place</div><div class="vc-ix-33"><button type="button" class="btn btn-p" data-act="Inv.openAdd()">+ Add Investment</button></div></div>`;return;}
    el.innerHTML=data.map(i=>{const r={Low:'b-ok',Medium:'b-warn',High:'b-err'};const hasBoth=i.amountInvested&&i.currentValue;const invLine=hasBoth?`<div class="vc-ix-115">Inv: <span class="sens">${i.currency} ${Math.round(i.amountInvested).toLocaleString()}</span> · Cur: <span class="sens">${i.currency} ${Math.round(i.currentValue).toLocaleString()}</span></div>`:'';return `<div class="entry" data-id="${i.id}"><div class="entry-main"><div class="entry-ic">${VC.investIcon(i.type,18)}</div><div class="entry-body"><div class="entry-name">${i.investmentName||i.broker||'Investment'}</div><div class="entry-sub">${i.broker||''} · ${i.type||''} · ${i.currency||''}</div><div class="entry-meta">${i.riskLevel?`<span class="badge ${r[i.riskLevel]||'b-muted'}">${i.riskLevel}</span>`:''} ${hasBoth?`<span class="vc-ix-231">${U.pnl(i.amountInvested,i.currentValue)}</span>`:i.currentValue?`<span class="badge b-acc">${U.fmt(i.currentValue)}</span>`:''} ${i.ownership==='business'?'<span class="badge b-warn">Business</span>':''} ${(i.tags||[]).slice(0,2).map(t=>`<span class="badge b-muted">${t}</span>`).join('')}</div>${invLine}</div><div class="entry-acts">${U.icb('star',{onclick:`Inv.fav('${i.id}')`,class:'fav'+(i.favorite?' on':'')})}${U.actsEditDel('Inv', i.id)}</div></div></div>`;}).join('');
    initSwipeDelete(el);
  },
  _pendingOwnerId: null,
  openAdd(prefill={}){
    Inv._pendingOwnerId = prefill.ownerId || null;
    const title = (prefill.ownerName||prefill._ownerName) ? `Add Investment — ${escHtml(prefill.ownerName||prefill._ownerName)}` : 'Add Investment';
    Modal.open(title,this.form(),`<button type="button" class="btn btn-g" data-act="Modal.close()">Cancel</button><button type="button" class="btn btn-p" data-act="Inv.save()">Save</button>`);setTimeout(()=>{const c=S.user.currency||'PKR';['if-inv','if-cur2'].forEach(id=>{const el=document.getElementById(id);if(el)U.numInput(el,c);});},60);
  },
  form(i={}){
    const isEdit=!!i.id;
    ...
    <!-- REQUIRED -->
    <div class="fg"><label class="fl">Investment Name *</label><input class="inp" id="if-name" list="invDL" placeholder="e.g. ENGRO, Bitcoin, Meezan Fund…" autocomplete="off" data-act-input="SMART_DB.fillInv(this.value)" value="${escAttr(i.investmentName||'')}"></div>
    ...
    <details${isEdit?' open':''} style="margin-top:10px">
      <summary class="vc-ix-111">
        <span class="vc-ix-5">More details</span><span class="vc-ix-18">▾</span>
      </summary>
      <div class="vc-ix-112">
        ...
        <div class="fr"><div class="fg"><label class="fl">Ticker / Symbol</label><input class="inp" id="if-tick" value="${escAttr(i.ticker||'')}" placeholder="ENGRO, BTC" data-act-input="Inv.lookupTicker(this.value)"><div id="if-tick-hint" style="font-size:11px;color:var(--text3);margin-top:3px;min-height:14px"></div></div><div class="fg"><label class="fl">Purchase Date</label><input class="inp" id="if-date" value="${escAttr(i.purchaseDate||'')}" type="date"></div></div>
        ...
        <label class="vc-ix-128"><input type="checkbox" id="if-fav" ${i.favorite?'checked':''}> Favourite</label>
      </div>
    </details>`;
  },
  async save(editId=null){const name=document.getElementById('if-name').value.trim(),broker=document.getElementById('if-broker').value.trim();if(!name&&!broker){Toast.show('Name or broker required','warning');return;}if(!editId){const dup=checkDuplicate('investment',{investmentName:name});if(dup.isDuplicate&&!await window.__vos_confirm(dup.message))return;}const lf=U.getLF();const _oid=editId?(S.investments.find(x=>x.id===editId)?.ownerId||'self'):(Inv._pendingOwnerId||'self');if(!editId)Inv._pendingOwnerId=null;const item={id:editId||U.id(),investmentName:name,broker,type:document.getElementById('if-type').value,riskLevel:document.getElementById('if-risk').value,currency:document.getElementById('if-cur').value,amountInvested:parseFloat((document.getElementById('if-inv').value||'').replace(/,/g,''))||0,currentValue:parseFloat((document.getElementById('if-cur2').value||'').replace(/,/g,''))||0,ticker:document.getElementById('if-tick').value.trim(),purchaseDate:document.getElementById('if-date').value,country:document.getElementById('if-cc').value,ownership:document.getElementById('if-own').value,...lf,notes:document.getElementById('if-notes').value.trim(),tags:U.getTags(),favorite:document.getElementById('if-fav').checked,ownerId:_oid,updatedAt:new Date().toISOString(),createdAt:editId?S.investments.find(x=>x.id===editId)?.createdAt:new Date().toISOString()};const auto=autoTags('investment',item);item.tags=[...new Set([...(item.tags||[]),...auto])];if(editId)S.investments=S.investments.map(x=>x.id===editId?item:x);else S.investments.push(item);Activity.log((editId?'Edited':'Added')+' investment',name||broker);Store.save();Modal.close();this.render();Toast.show(`${editId?'Updated':'Added'}: ${name||broker}`,'success');if(!editId)promptAddAnother('Investment','Inv.openAdd');},
  edit(id){const i=S.investments.find(x=>x.id===id);if(!i)return;Modal.open('Edit Investment',this.form(i),`<button type="button" class="btn btn-g" data-act="Modal.close()">Cancel</button><button type="button" class="btn btn-d btn-sm" data-act="Inv.del('${id}',true)">Delete</button><button type="button" class="btn btn-p" data-act="Inv.save('${id}')">Update</button>`);setTimeout(()=>{[['if-type',i.type||'Stocks'],['if-risk',i.riskLevel||'Medium'],['if-cur',i.currency||'GBP'],['if-cc',i.country||'GB'],['if-own',i.ownership||'personal']].forEach(([el,v])=>{const e=document.getElementById(el);if(e)e.value=v;});U.setLF(i);const c=i.currency||'GBP';['if-inv','if-cur2'].forEach(id=>{const el=document.getElementById(id);if(el)U.numInput(el,c);});},60);},
  lookupTicker(ticker){
