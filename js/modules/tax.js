...
const Tax = {
  ...
  <title>Tax Report — VaultOS</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, Arial, sans-serif; background: #fff; color: #1a1a2e; }
    .page { max-width: 600px; margin: 0 auto; padding: 40px 32px; }
    .header { background: linear-gradient(135deg, #1a237e, #283593); color: white; padding: 28px 32px; border-radius: 16px; margin-bottom: 28px; }
    .header h1 { font-size: 24px; font-weight: 900; margin-bottom: 4px; }
    .header .sub { font-size: 13px; opacity: .8; }
    .meta { background: #f1f3f5; border-radius: 10px; padding: 14px 16px; margin-bottom: 16px; font-size: 13px; color: #555; line-height: 1.7; }
  ...
  <div class="page">
    ...
    <div class="meta">
      Country: ${cfg?.name || ''} &nbsp;|&nbsp; Filing: ${filing?.name || ''} &nbsp;|&nbsp; Tax Year: ${filing?.year || ''}
    </div>
    <div class="section">
      <div class="section-title">Tax Calculation</div>
      ${report.innerText.trim().split('\n').filter(l => l.trim() && !l.includes('Print') && !l.includes('🖨') && !l.includes('Tax Report')).map(l => {
    ...
    <div class="footer">
      ...
      For guidance only — consult a qualified tax professional or accountant for advice specific to your situation.
    </div>
