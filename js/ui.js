...
const SettingsNav = {
  ...
  _import() {
    const llmOn = !!(S.user.llmEnabled && S.user.llmApiKey);
    return `<div class="set-sec"><div class="set-title">🤖 Optional LLM Assist</div><div class="set-card">
      <div class="si"><div class="sil"><div class="name">Enable LLM parsing</div><div class="desc">Uses your API key for Smart Add &amp; Smart Import. Smart Parser remains the offline default.</div></div><label class="tog"><input type="checkbox" ${S.user.llmEnabled?'checked':''} onchange="S.user.llmEnabled=this.checked;Store.save();SettingsNav.show('import')"><span class="ts"></span></label></div>
      <div class="si"><div class="sil"><div class="name">Provider</div><div class="desc">Anthropic or OpenAI-compatible</div></div><select class="inp btn-sm" style="width:auto;padding:5px 9px" onchange="S.user.llmProvider=this.value;Store.save()"><option value="anthropic" ${(S.user.llmProvider||'anthropic')==='anthropic'?'selected':''}>Anthropic</option><option value="openai" ${S.user.llmProvider==='openai'?'selected':''}>OpenAI</option></select></div>
      <div style="padding:12px 14px"><label class="fl">API Key (stored on device only)</label><input class="inp" type="password" id="llm-key-inp" placeholder="${llmOn?'••••••••':'Paste key — never committed to git'}" autocomplete="off" onchange="S.user.llmApiKey=this.value;Store.save()"><div style="font-size:11px;color:var(--text3);margin-top:8px;line-height:1.5">Rotate keys if exposed. VaultOS never sends data except to the provider you choose.</div><button class="btn btn-g btn-sm" style="margin-top:8px" onclick="LlmAssist.clearKey();document.getElementById('llm-key-inp').value='';Toast.show('API key cleared')">Clear Key</button></div>
    </div></div>
